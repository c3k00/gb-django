...
def create_order(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.save()  # Сначала сохраняем объект Order, чтобы получить первичный ключ

            # Получаем продукты из POST данных и добавляем их к заказу
...
def ordered_products_view(request):
    ...
    products_last_7_days = {product for order in orders_last_7_days for product in order.products.all()}
    products_last_30_days = {product for order in orders_last_30_days for product in order.products.all()}
    products_last_year = {product for order in orders_last_year for product in order.products.all()}
